...
def aa_min_distances():
    ...
    aa_atoms = [['ALA', ['CA']], ['CYS', ['CA']], ['ASP', ['CA']],
                ['GLU', ['CA']], ['GLY', ['CA']], ['SER', ['CA']],
                ['ARG', ['CA']], ['LYS', ['CA']],
                ['PHE', ['CA']],
                ['TYR', ['CA']],
                ['ASN', ['CA']],
...
def min_value():
    reader = csv23.open_reader('interface distances CA.csv')
    matrix = []
    with reader as reader:
        for row in reader:
            matrix.append(row)
    ...
    for i in range(len(matrix)):
        if matrix[i][0] < 24 and matrix[i][1] >= 24:
            if matrix[i][2] < min and matrix[i][2] != 0:
                min = matrix[i][2]
                x = matrix[i][0]
                y = matrix[i][1]
    print('{},{} Min {} '.format(x, y, min))
